...
When `brain.json.project.type === "game"` or `.uproject` exists, activate game-specific help.

### Context States
| State | Indicators | Help Focus |
|-------|------------|------------|
| C++ Development | Editing .cpp/.h | Macros, patterns |
| Blueprint Editing | current_blueprint set | Nodes, debugging |
| Level Design | current_map set | Optimization, lighting |
| Build/Deploy | build_target active | Packaging, platforms |
| Debugging | Errors present | Error translation |

### State-Based Help

#### C++ Context
- Quick ref: UCLASS, UPROPERTY, UFUNCTION specifiers
- Watch out: Include in .cpp, forward declare in .h
- Suggest: /code, /debug

#### Blueprint Context
- Quick ref: F9 breakpoint, F10 step over
- Watch out: Avoid cast in loops, use Events
- Suggest: /run, /debug

#### Level Context
- Quick ref: Lighting, streaming, collision
- Watch out: Draw calls < 2000, use LODs
- Suggest: /audit, /run

### Docs/ 5-Phase Guidance
| Phase | Focus | Commands |
|-------|-------|----------|
| Requirements | User stories | /plan |
| Design | Architecture | /visualize |
| Planning | Task breakdown | /plan |
| Implementation | Code/assets | /code, /run |
| Testing | Validation | /test, /audit |
