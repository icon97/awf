...
- **🔧 Game-Specific Workflows** - New workflows: /build-ue, /blueprint, /assets, /docs-sync, /multiplayer
- **📚 Docs/ Integration** - Full support for 5-phase documentation structure
- **🇻🇳 Vietnamese Terminology** - Newbie-friendly explanations for game dev concepts
...
AWF automatically detects UE5 projects by checking for:
- `*.uproject` file in project root
- `Source/` folder with C++ code
- `Content/` folder with assets
- `Config/` folder with INI files

When detected, all workflows switch to **Game Dev Mode** with UE5-specific features.

### Project Types

| Type | Detection | Features |
|------|-----------|----------|
| web | package.json, index.html | Standard web workflows |
| mobile | AndroidManifest.xml, Info.plist | Mobile-specific workflows |
| game | *.uproject | UE5 Game Dev Mode |

### Game-Specific Workflows

| Workflow | Purpose | Trigger |
|----------|---------|---------|
| /build-ue | UE5 build management | /build-ue, /build |
| /blueprint | Blueprint management | /blueprint, /bp |
| /assets | Content folder management | /assets, /content |
| /docs-sync | Documentation sync | /docs-sync, /docs |
| /multiplayer | Multiplayer development | /multiplayer, /mp |

### Enhanced Workflows

All existing workflows are enhanced with Game Dev Mode:

| Workflow | Game Dev Enhancement |
|----------|---------------------|
| /init | Detects UE5 project, sets game_context |
| /plan | Includes UE5 architecture patterns |
| /code | C++/Blueprint dual support |
| /run | PIE/Standalone launch options |
| /test | Automation testing support |
| /deploy | Platform packaging (Win64, PS5, etc.) |
| /visualize | Blueprint/C++ diagrams |
| /debug | UE5 debugging tools |
| /audit | Content folder auditing |
| /refactor | Blueprint/C++ refactoring |

### Game Context

Session tracks game-specific context:
...
```json
{
  "game_context": {
    ...
    "engine_version": "5.3",
    "project_name": "MyGame",
    "target_platforms": ["Win64", "PS5"],
    "modules": ["MyGame", "MyGameEditor"],
    "plugins": ["OnlineSubsystem", "Niagara"]
  }
}
```
...
AWF supports the 5-phase documentation structure:
...
Use `/docs-sync` to keep documentation synchronized with code.
...
| Actor | Object có thể đặt trong Level |
| Component | Thành phần gắn vào Actor |
| Replication | Đồng bộ dữ liệu multiplayer |

### Quick Start (Game Dev)

1. Open UE5 project folder
2. Run `/init` - AWF detects game project
3. Run `/plan` - Create development plan
4. Run `/code` - Start coding with C++/Blueprint support
5. Run `/build-ue` - Build project
6. Run `/run` - Test in PIE or Standalone
