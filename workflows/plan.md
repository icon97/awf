...
### 0.5. Game Project Detection
```
if brain.json.project.type === "game" OR exists("*.uproject"):
    → Activate Game Feature Discovery mode
    → Load game_engine context from brain.json
    → Use game-specific phase templates
```

### Game Feature Types:
| Type | Examples | Typical Phases |
|------|----------|----------------|
| **Gameplay System** | Physics, AI, Combat, Vehicle | Core → Tuning → Polish |
| **UI/HUD** | Minimap, Menus, HUD elements | Design → UMG → Integration |
| **Multiplayer** | Matchmaking, Replication, Sync | Backend → Client → Testing |
| **Content** | Levels, Vehicles, Characters | Design → Assets → Integration |
| **Progression** | XP, Achievements, Unlocks | Data → Logic → UI |
| **Audio** | SFX, Music, Voice | Design → Implementation → Mix |

### Feature Discovery Questions (Game):
```
"🎮 Feature này thuộc loại nào?"

1️⃣ **Gameplay System** - Cơ chế chơi game (physics, AI, combat)
2️⃣ **UI/HUD** - Giao diện người dùng trong game
3️⃣ **Multiplayer** - Tính năng chơi nhiều người
4️⃣ **Content** - Nội dung game (maps, characters, items)
5️⃣ **Progression** - Hệ thống tiến trình (XP, achievements)
6️⃣ **Audio** - Âm thanh, nhạc nền
7️⃣ **Khác** - Mô tả cho em
```

### Blueprint vs C++ Decision:
```
"Code feature này bằng gì?"

🔵 **Blueprint** khi:
- Prototype nhanh, thử nghiệm ý tưởng
- Logic đơn giản, ít tính toán
- Designer cần chỉnh sửa
- UI/UX interactions

🔴 **C++** khi:
- Performance critical (physics, AI)
- Complex algorithms
...
- Expose UPROPERTY/UFUNCTION cho designers
```
...
### Output Location (Game Projects):
```
if exists("Docs/features/"):
    → Output to: Docs/features/[feature-name]/
    → Structure:
      ...
      ├── planning/
      │   └── plan.md          # ← /plan outputs here
      ├── implementation/
      │   └── README.md        # Implementation notes
      └── testing/
          └── README.md        # Test plans
else:
    → Fallback to: plans/[timestamp]-[feature-name]/
```

### Terminology cho newbie:
| Thuật ngữ | Giải thích đời thường |
|-----------|----------------------|
| Subsystem | Hệ thống con quản lý 1 việc (như quản lý xe, inventory) |
| Replication | Đồng bộ data giữa server và client (multiplayer) |
| Tick | Code chạy mỗi frame (60 lần/giây) |
| GAS | Gameplay Ability System - hệ thống skill của UE5 |
| DataTable | Bảng dữ liệu tĩnh (items, stats, configs) |
| SaveGame | Class lưu trữ progress của player |
| Interface | Hợp đồng giữa các class (không cần biết class cụ thể) |

### Game Design Document (GDD) Section:
```
Khi plan game feature, tự động tạo GDD section:

## [Feature Name] - GDD

### Overview
- **Feature Type**: [Gameplay/UI/Multiplayer/Content/Progression]
- **Priority**: [P0/P1/P2]
- **Complexity**: [Low/Medium/High]

### Player Experience
- What does the player see/do?
- How does it feel?
