...
if exists("*.uproject") OR brain.json.project.type === "game":
    → Chế độ: UE5 Recap
    → Include: Game-specific summary fields
    → Show: Current map, build status
```
...
🎯 Build Target: [Win64/Android/iOS]
📦 Last Build: [Success/Failed] at [timestamp]
...
📈 Session Stats:
- Lines of C++ changed: [count]
- Blueprints modified: [count]
- Assets added: [count]
...
| Status | Icon | Meaning |
|--------|------|---------|
| Success | ✅ | Build completed, no errors |
| Warning | ⚠️ | Build completed with warnings |
| Failed | ❌ | Build failed, errors present |
...
#### Map Information:
```
🗺️ Current Map: [MapName]
├── Path: Content/Maps/[MapName].umap
├── Size: [X] x [Y] units
├── Actors: [count]
├── Lights: [count]
└── Last Modified: [timestamp]

📍 Recent Maps Worked On:
1. [Map1] - [last access time]
2. [Map2] - [last access time]
3. [Map3] - [last access time]
```
...
#### Last PIE Session:
```
🎮 Performance Snapshot (PIE)

FPS: [avg] (min: [min], max: [max])
Frame Time: [avg]ms
Draw Calls: [count]
Triangles: [count]
Memory: [MB] MB
...
- FPS: [+/-X] from target 60
- Memory: [+/-X MB] from limit
```
...
## 🔧 Technical Changes
- **C++ Files**: [count] modified
- **Blueprints**: [count] modified
- **Assets**: [count] added/modified
...
|-----------|----------------------|
| Recap | Tóm tắt những gì đã làm |
| PIE | Play In Editor - chạy thử game trong Editor |
| Build Target | Platform đang build (Win64, Android, etc.) |
| Session | Phiên làm việc (từ lúc mở đến lúc đóng project) |
| Baseline | Mức hiệu năng chuẩn để so sánh |
