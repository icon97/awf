...
### 0.4. UE5 Project Detection

```
if exists("*.uproject") OR brain.json.project.type === "game":
    → Chế độ: UE5 Game Development
    → Load: Docs/_standards/code-standards.md (if exists)
    → Apply: Epic C++ Coding Standard
...
### Code Mode Selection:
```
"🎮 Anh muốn code bằng gì?"

A) 🔴 **C++** - Performance, complex logic
   - Core systems, subsystems
   - Performance-critical code
   - Reusable base classes

B) 🔵 **Blueprint** - Visual, rapid prototyping
   - Quick iterations
   - Designer-friendly
   - UI logic, simple gameplay

C) 🟣 **Hybrid** (Recommended)
   - C++ base class với core logic
   - Blueprint subclass cho customization
   - Best of both worlds
```
...
| Rule | Example |
|------|---------|
| **Prefix U** | `UMySubsystem` - UObject-derived classes |
| **Prefix A** | `AMyActor` - Actor-derived classes |
| **Prefix F** | `FMyStruct` - Structs |
| **Prefix E** | `EMyEnum` - Enums |
| **Prefix I** | `IMyInterface` - Interfaces |
| **Prefix T** | `TMyTemplate` - Templates |
| **Prefix b** | `bIsEnabled` - Boolean variables |
...
└── Private/          # Implementation (.cpp)
    └── [System]/     # Match Public structure
```

### UE5 Macros (UCLASS, UPROPERTY, UFUNCTION):

#### UCLASS Specifiers:
```cpp
UCLASS(BlueprintType)           // Can be used in Blueprint
UCLASS(Blueprintable)           // Can be subclassed in Blueprint
UCLASS(Abstract)                // Cannot be instantiated
UCLASS(Config=Game)             // Reads from config files
```
...
UPROPERTY(Replicated)           // Synced in multiplayer
UPROPERTY(SaveGame)             // Included in save files
```

#### UFUNCTION Specifiers:
```cpp
UFUNCTION(BlueprintCallable)    // Can call from Blueprint
UFUNCTION(BlueprintPure)        // No side effects
UFUNCTION(BlueprintImplementableEvent)  // Override in BP
UFUNCTION(BlueprintNativeEvent) // C++ default, BP override
UFUNCTION(Server, Reliable)     // Server RPC
UFUNCTION(Client, Reliable)     // Client RPC
UFUNCTION(NetMulticast)         // All clients RPC
```

### Hot Reload vs Live Coding:

| Feature | Hot Reload | Live Coding |
|---------|------------|-------------|
| **Trigger** | Ctrl+Alt+F11 | Ctrl+Alt+F11 (UE5.1+) |
| **Speed** | Slower | Faster |
| **Stability** | Less stable | More stable |
| **Changes** | Full recompile | Incremental |
| **Recommendation** | Legacy | ✅ Preferred |

### Khi nào cần Full Restart:
- Thay đổi UCLASS/USTRUCT layout
- Thêm/xóa UPROPERTY
- Thay đổi constructor
- Thay đổi header files

### UE5 Compilation Commands:

| Action | Command/Shortcut |
|--------|------------------|
| **Build** | Ctrl+Shift+B (VS) / Ctrl+B (Rider) |
| **Build & Run** | F5 |
...
```bash
# Windows
Engine\Build\BatchFiles\Build.bat [ProjectName] Win64 Development

# Full rebuild
Engine\Build\BatchFiles\Build.bat [ProjectName] Win64 Development -clean
```
...
|-------|-----------|-----|
| `LNK2019` | Thiếu implementation | Implement function trong .cpp |
| `C2065` | Undeclared identifier | Thêm #include |
| `C2511` | Overloaded function not found | Check virtual function signature |
| `C4430` | Missing type specifier | Forward declare hoặc #include |

### Terminology cho newbie:

| Thuật ngữ | Giải thích đời thường |
|-----------|----------------------|
| UCLASS | Đánh dấu class cho UE5 nhận diện |
| UPROPERTY | Biến hiện trong Editor, có thể edit |
| UFUNCTION | Hàm gọi được từ Blueprint |
| TObjectPtr | Con trỏ thông minh của UE5 (thay raw pointer) |
| TWeakObjectPtr | Con trỏ yếu, không giữ object sống |
| TArray | Mảng động của UE5 (như std::vector) |
| TMap | Dictionary của UE5 (như std::map) |
| FString | String của UE5 |
| FName | String tối ưu cho so sánh |
| FText | String cho UI (hỗ trợ localization) |
