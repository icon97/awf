...
### Giai đoạn 2.5: UE5 Project Setup

**Khi User chọn F) Game:**

#### Tech Stack (Auto-selected):
| Component | Value |
|-----------|-------|
...
#### UE5 Project Detection:
```
if exists("*.uproject"):
    → Existing UE5 project detected
    → Scan Source/, Content/, Plugins/
    → Auto-populate brain.json game_engine section
else:
    → Guide user to create project via UE5 Editor first
...
```
[ProjectName]/
├── Source/[ProjectName]/     # C++ code
│   ├── Public/               # Headers (.h)
│   └── Private/              # Implementation (.cpp)
├── Content/                  # Assets, Blueprints, Maps
├── Config/                   # INI configuration files
├── Plugins/                  # Custom and third-party plugins
├── Docs/                     # Documentation (5-phase structure)
│   ├── features/             # Feature documentation
│   ├── _architecture/        # Architecture docs
│   ├── _standards/           # Coding standards
│   ├── _templates/           # Doc templates
│   └── _cross-reference/     # Cross-reference maps
└── .brain/                   # AWF context files
    ├── brain.json
    ├── session.json
    └── preferences.json
```

#### UE5-specific .gitignore:
```
# UE5 Generated
Binaries/
Intermediate/
...
| Actor | Vật thể trong game world |
| Component | Phần nhỏ gắn vào Actor (như bánh xe gắn vào xe) |
| Level/Map | Màn chơi, bản đồ |
| PIE | Play In Editor - chạy thử game ngay trong Editor |
| Content | Thư mục chứa assets (hình ảnh, âm thanh, models) |
| Module | Một phần code C++ độc lập |
| Plugin | Tính năng mở rộng có thể bật/tắt |
| Subsystem | Hệ thống con quản lý 1 việc cụ thể |

#### Khởi tạo Docs/ Structure:
```
Docs/
...
### 🎮 Game Project Next Steps:
1. `/plan` - Lên kế hoạch feature đầu tiên
2. `/code` - Bắt đầu code C++ hoặc Blueprint
3. `/run` - Chạy thử game trong Editor (PIE)
4. `/build-ue` - Build game cho platform cụ thể
