...
if exists("*.uproject") OR brain.json.project.type === "game":
    → Chế độ: UE5 UI/UX Design
    → Load: game_engine config from brain.json
    → Show: UMG/HUD design options
```
...
|--------|----------|---------|
| **Canvas Panel** | Root container, absolute positioning | Main HUD |
| **Vertical/Horizontal Box** | Linear layout | Menu items |
| **Overlay** | Stack widgets on top | Health bar over portrait |
| **Grid Panel** | Grid layout | Inventory slots |
| **Scroll Box** | Scrollable content | Chat log, leaderboard |
| **Size Box** | Fixed size container | Icons, buttons |
| **Border** | Background + padding | Panels, cards |
| **Button** | Clickable element | Menu buttons |
| **Text Block** | Display text | Labels, scores |
| **Image** | Display texture | Icons, backgrounds |
| **Progress Bar** | Show progress | Health, XP, loading |

### HUD Design Patterns:

#### Racing Game HUD (PrototypeRacing):

```
┌─────────────────────────────────────────────┐
│ [Position]              [Lap: 1/3]    [Time]│
│                                             │
│                                             │
│                                             │
│ [Minimap]                                   │
│                                             │
│                                             │
│ [Speed: 120 km/h]    [Nitro Bar]    [Gear] │
└─────────────────────────────────────────────┘
```

#### Common HUD Elements:

| Element | Position | Widget Type |
|---------|----------|-------------|
| Health/Shield | Top-left | Progress Bar + Overlay |
| Minimap | Bottom-left or Top-right | Image + Canvas |
| Score/Timer | Top-center | Text Block |
| Ammo/Resources | Bottom-right | Text + Image |
| Crosshair | Center | Image |
| Notifications | Top-center | Vertical Box |
| Quick Slots | Bottom-center | Horizontal Box |
...
│   ├── WBP_Minimap (Bottom-Left)
│   └── Horizontal Box (Bottom-Center)
│       ├── WBP_SpeedDisplay
│       ├── WBP_NitroBar
│       └── WBP_GearIndicator
```
...
| Prefix | Type | Example |
|--------|------|---------|
| `WBP_` | Widget Blueprint | WBP_MainHUD |
| `WBP_` | Reusable component | WBP_HealthBar |
| `Txt_` | Text Block | Txt_Score |
| `Img_` | Image | Img_Background |
| `Btn_` | Button | Btn_Start |
| `Prg_` | Progress Bar | Prg_Health |

### Rive Integration (Advanced Animation):

#### Setup:
1. Install Rive plugin from Marketplace
2. Import .riv files to Content/
3. Create Rive Widget in UMG

#### Use Cases:

| Animation Type | Rive Advantage |
|----------------|----------------|
| Loading spinners | Smooth, small file size |
| Character portraits | Interactive states |
| Button effects | Hover/click animations |
...
```
Create Widget (WBP_RiveAnimation)
    → Set Rive File
    → Play Animation
    → Bind to Events (OnComplete, OnStateChange)
```

### Responsive Design (Multi-Resolution):

#### Anchors:

| Anchor | Use Case |
|--------|----------|
| Top-Left | Health, minimap |
| Top-Center | Notifications |
| Top-Right | Score, timer |
| Center | Crosshair, dialogs |
| Bottom-Left | Chat |
...
- Rule: Shortest Side
- Curve: Custom (adjust for mobile)
```

#### Safe Zone:

```
Use Safe Zone widget for mobile/TV
- Accounts for notches, rounded corners
- Essential for console certification
```
...
|-----------|----------------------|
| UMG | Unreal Motion Graphics - hệ thống UI của UE5 |
| Widget | Thành phần UI (nút, text, hình ảnh) |
| HUD | Heads-Up Display - UI trong game (health, minimap) |
| Canvas Panel | Container cho phép đặt widget ở vị trí bất kỳ |
| Anchor | Điểm neo widget khi resize màn hình |
| Binding | Kết nối data với UI (auto update) |
| Rive | Tool tạo animation vector cho UI |
| Safe Zone | Vùng an toàn cho mobile/TV (tránh notch) |
